...
public class DustUtils {
	...
	public static void send(DustObject target, Enum<?> msgId, DustObject msg) {
		target.send(msgId, msg, true, null);
	}
	
	public static void send(DustObject target, Enum<?> msgId) {
		target.send(msgId, null, true, null);
	}
	...
	public static final Iterable<? extends DustObject> NO_MEMBER = new Iterable<DustObject>() {
		Iterator<DustObject> it = new EmptyIter<DustObject>();
		@Override
		public Iterator<DustObject> iterator() {
			return it;
		}
	};	
	
	public static class ArrayIter<Target> implements Iterator<Target>, Iterable<Target> {
		Target[] data;
		int idx;
		
		public ArrayIter(Target[] data) {
			this.data = data;
			idx = 0;
		}
		...
		@Override
		public boolean hasNext() {
			return ((null != data) && (idx < data.length));
		}
		...
		@Override
		public void remove() {
			throw new DustRuntimeException("Utils.ArrayIter", "remove() is not supported");
		}
	}
